...
## Development Status
**WARNING**: The package is in active development. All implementations are experimental and APIs may change. See [Get started](articles/multimod.html) for an overview of current functionality and [Development Roadmap](articles/roadmap.html) for implementation status and future plans.
...
│ {linopy} (runtime capture)  │ → {read_linopy()}
...
library(multimod)
mod <- multimod::example_models$energyRt$multimod
class(mod)
```
...
#### {Read Python models (linopy/PyPSA)}

> Not currently shipped. The experimental linopy/PyPSA readers live in
> `drafts/` and are not part of the package; the example below is
> illustrative of the planned API.
```r
# Requires reticulate and Python packages
library(reticulate)

# Read linopy model (runtime capture)
linopy <- import("linopy")
m <- linopy$Model()
# ... build model ...
mod <- read_linopy(m)

# Read PyPSA network model
pypsa <- import("pypsa")
n <- pypsa$Network()
# ... configure network ...
